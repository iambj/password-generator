// Generates "entropy" by moving the mouse around.
var targID = "data";
...
var entropyLength = 1024 * 0.5; //1kbyte in bits
var pool = "";
...
function generate() {
    if (pool.length >= entropyLength) {
        var one = pool.charAt(Math.floor(Math.random() * pool.length));
        var two = pool.charAt(Math.floor(Math.random() * pool.length));
        var three = pool.charAt(Math.floor(Math.random() * pool.length));
        var four = pool.charAt(Math.floor(Math.random() * pool.length));
        results.innerHTML += "" + one + two + pickWord() + three + four + "</br>";
    }
}
